...
DELIMITER / / CREATE FUNCTION fGetCostPerMinute() RETURNS FLOAT DETERMINISTIC BEGIN RETURN 1;

END / / DELIMITER;

DROP FUNCTION IF EXISTS fCalculateCity;

DELIMITER / / CREATE FUNCTION fCalculateCity (pNumber VARCHAR(25)) RETURNS INTEGER NOT DETERMINISTIC READS SQL DATA BEGIN DECLARE vIdCity INTEGER DEFAULT 0;
...
IF (vIdCity = 0) THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Error finding city to match prefix of number',
    MYSQL_ERRNO = '1';
...
DELIMITER / / CREATE FUNCTION fCalculateLine (pNumberLine VARCHAR(50)) RETURNS INTEGER NOT DETERMINISTIC READS SQL DATA BEGIN DECLARE vIdLine INTEGER DEFAULT 0;
...
GROUP BY
    idLine,
    numberLine INTO vIdLine;

IF (vIdLine = 0) THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Error finding line for number specified',
    MYSQL_ERRNO = '1';
...
DELIMITER / / CREATE FUNCTION fCalculatePrice (pIdOrigin INTEGER, pIdDestination INTEGER) RETURNS INTEGER NOT DETERMINISTIC READS SQL DATA BEGIN DECLARE vPrice FLOAT DEFAULT 0;
...
FROM
    rates r
WHERE
    r.idCityOrigin = pIdOrigin
    AND r.idCityDestination = pIdDestination
GROUP BY
    r.pricePerMinute INTO vPrice;

IF (vPrice = 0) THEN SIGNAL SQLSTATE '45000'
SET
    MESSAGE_TEXT = 'Error finding price per minute to apply to new call',
    MYSQL_ERRNO = '1';
...
DELIMITER / / CREATE FUNCTION fCalculateUser (pIdLine INTEGER) RETURNS INTEGER NOT DETERMINISTIC READS SQL DATA BEGIN DECLARE vIdUser INTEGER DEFAULT 0;
...
WHERE
    pl.idLine = pIdLine
GROUP BY
    pl.idUser INTO vIdUser;

IF(vIdUser = 0) THEN
SET
    @asd = CONCAT('Error findig user of line: ', pIdLine);
...
DELIMITER / / CREATE TRIGGER tbiCalculateInsertCall BEFORE
INSERT
    ON calls FOR EACH ROW BEGIN
SET
    NEW.idCityOrigin = fCalculateCity(NEW.numberOrigin);

SET
    NEW.idCityDestination = fCalculateCity(NEW.numberDestination);

SET
    NEW.pricePerMinute = fCalculatePrice(NEW.idCityOrigin, NEW.idCityDestination);

SET
    NEW.costPerMinute = fGetCostPerMinute();

SET
    NEW.priceTotal = (NEW.durationInSeconds / 60) * NEW.pricePerMinute;

SET
    NEW.idLineOrigin = fCalculateLine(NEW.numberOrigin);

SET
    NEW.IdLineDestination = fCalculateLine(NEW.numberDestination);
...
DELIMITER / / CREATE PROCEDURE pliquidateActiveLines () BEGIN DECLARE vIdLine INTEGER;

DECLARE vFinished INTEGER DEFAULT 0;

DECLARE curLines CURSOR FOR
SELECT
    idLine
FROM
    phoneLines pl
WHERE
    pl.statusLine = 'ACTIVE'
GROUP BY
    idLine;
...
WHILE (vFinished = 0) DO START TRANSACTION;
...
CREATE EVENT eLiquidateActiveLines ON SCHEDULE EVERY 10 MINUTE STARTS CURRENT_TIMESTAMP ENDS CURRENT_TIMESTAMP + INTERVAL 48 HOUR ON COMPLETION PRESERVE DO call pliquidateActiveLines();
